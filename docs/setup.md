...
### G8 — Petri multi-model mode requires Inspect AI 0.3.263+ and correct role mapping

The deployed provider image currently contains inspect-ai 0.3.251, which rejects
per-role model overrides. Inspect AI 0.3.263 is the current 0.3.x release and
supports model specs with a per-role `base_url`; a per-role API key must be
passed as a provider/model argument or supplied through the provider's
environment/Secret mechanism. `api_key` is not a `GenerateConfig` field.
Consequently, upgrading the provider is necessary, but the adapter must also
map `judge_api_key`/`auditor_api_key` to the model credential mechanism rather
than placing them in `GenerateConfig`.
...
| `Unknown GenerateConfig field: base_url` or `api_key` | Old provider, or adapter placed model fields in `GenerateConfig` (G8) | Upgrade/rebuild the provider and verify role mapping, or use single-endpoint mode |
